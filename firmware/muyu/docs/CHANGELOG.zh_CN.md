...
- 芽芽对话增加倾听、思考、说话和待机动作；沿用固定文案，用像素点和短条反馈状态，不加入全文字幕大字库。
- 嘴型与短条使用成功提交播放的PCM幅度，独立于麦克风level，及时清除过期或取消的输出。音频缓冲、协议与40 KiB工作栈保持。
- 使用紧凑气泡，不增加图片、字体、canvas或后台任务；菜单/熄屏后重置动作，保留激活与错误可读性、养成原绘制与存档隔离。
- 以0.6.0为基线，限制应用镜像增长8 KiB、聊天/角色/语音组件静态RAM合计增长256 B。实机时序、峰值RAM和长聊稳定性仍需单独验收。

## 0.6.0 - 独立芽芽养成与对话

- Yaya Pet 与 Yaya Chat 分别作为菜单应用，只共享像素美术；聊天不读写养成存档，现有云端七喜配置不变。
- 共享房间／角色与状态对话卡片替代小智圆球，复用唯一 4,288 字节角色缓冲；语音服务、TLS、Opus 和 40 KiB 工作栈不改。
- 应用区扩大到 240x290，移除常驻底栏；保留激活／错误、键盘帮助和按键故障提示。
- 移除 Storage、About、设备／运行内存／容量子页及蓝牙，构建关闭 BT／NimBLE／共存；保留 Wi-Fi 配网、音量、亮度、超时和熄屏模式。
- 保留开发容量／链接报告、保护布局及预留门禁；完整验证加入统一 `--preview`。新版实机验收和内存节省实测仍待完成。

## 0.5.3 - 小智自动连续对话

- 按一次 OK 开始，服务端自动断句，回答播完后自动继续听。
- TTS 开始才停止采集，下一轮前清理麦克风旧采样。
- 增加经测试的对话状态机、取消／超时控制和脱敏传输诊断。
- 电脑端同连接两轮自动问答通过；0.5.2 实机录放通过但仍有稳定性待测项，0.5.3 新固件实机验收待执行。
- 保留 40 KiB 工作栈、编解码参数、系统 Wi-Fi 与受保护存储布局。

## 0.5.2 - 小智编解码诊断

- 语音联网和获取 I2S 前，用固定 PCM 真正执行编码与解码。
- 用 40 KiB 工作栈测量余量，保持 Opus 参数不变，记录首次真实采集／解码边界。
- 增加有限时的电脑端真实语音探测工具、脱敏统计和离线协议测试。
- 0.5.1 实机已确认编码器创建后小智任务栈溢出；本诊断版仍需实机验证。

## 0.5.1 - 小智运行内存

- 针对实机 C3 Opus 编码器分配失败，对齐上游 Wi-Fi／TLS 内存配置。
- 同时只保留一个方向的编解码器，空闲、思考或取消时释放。
- 保留具体初始化错误，报告内部空闲堆、最大连续块与栈余量。
- 增加 10,000 轮资源归属及故障注入回归；新固件实机验收待执行。

## 0.5.0 - 独立小智测试版

- 新增复用系统 Wi-Fi 的小智语音、受保护身份与有限时的按键半双工会话。
- 固件不再链接木鱼；像素宠物先保持离线，后续对接语音。
- 后续实机发现 Opus 编码器分配失败，由 0.5.1 替代。

## 0.4.1 - 紧凑系统状态栏

- Wi-Fi、BLE 改为电量旁的小图标，移除单独占行的联网文字。
- 应用容器从 240×235 扩大到 240×265；调整宠物与木鱼间距，不放大图片素材。
- 设置保留详细无线状态，预览覆盖关闭、在线、连接中、未配置、错误，以及长名称和 100% 电量。
- 宠物存档格式、玩法规则、设备引脚和受保护分区保持兼容。

## 0.4.0 - 宠物 0.1.0 测试版

- 新增轻操作像素宠物：自动生活、喂食／陪玩／休息、升级和一次进化。
- 两份小型 CRC 存档、事务失败保护，菜单／熄屏／切换时暂停。
- 复用 16 色房间、小角色缓冲和子集字体，强制为后续语音预留容量。
- 新增独立模块 tick 和带会话校验的 Avatar 契约，供后续小智整合。
- IDA 规划为独立语音应用入口；本版不包含两种联网对接。
- 增加模型／存档／素材测试与真实 LVGL 切换预览；实机验收待完成。

## 0.3.0

- 设备小键盘配网替代手机配网，限时连接、成功后保存。
- 应用列表只放自定义应用，设备状态/容量归系统，新增熄屏继续运行。
- 清单生成应用注册表，提供脚手架，两次链接校验每应用实际容量。
- 固化小智/尖塔远征源码适配与整包交付链路，保留 Flash 保护布局。


## 2026-09-13 - Passport 多应用底座 0.2.0

- 新增常驻应用底座、最近应用恢复、共享 Wi-Fi 配网、BLE 广播管理、公共设置与息屏。
- 新增会话资源取消、OK 松手确认与主机/UI 压力测试。
- 设置 NVS 位于 0x310000，原应用上限与设备身份区保持不变。
- 实机验收待完成，见[清单](platform-acceptance.zh_CN.md)。

## Unreleased

- 本项目新增离线木鱼 MVP：任意按键按下计数一次、合成木鱼声、有界混音、敲击动画、本次计数、电量显示、主机逻辑测试与真实 LVGL 预览检查。
