...
沿用现有字体的固定状态词。紧凑气泡在思考时显示像素点，倾听或回答时显示三段短条；不加入全文字幕、大字库、新图片、canvas或动画任务。聊天动作控制器改变眨眼、视线、叶耳、呼吸和嘴型，养成原绘制与存档仍独立。

说话幅度取自成功提交扬声器播放的已解码PCM，不使用麦克风幅度。静音、取消或过期输出闭嘴；语音层用自己的时钟计算年龄，250ms起归零，画面在正常约200ms的绘制周期中跟随更新，调度或LVGL锁繁忙可能延后。这是近似声音活动，不是音素口型同步或校准音量计。隐藏页面停止绘制，返回时重置动作。音频生命周期、协议、缓冲与40 KiB工作栈保持。

第二次链接后的容量门禁以`apps/chat-motion-budget.json`固定的0.6.0为基线，限制应用镜像增长8 KiB、聊天/角色/语音组件静态RAM合计增长256 B。这些数值不含动态堆、栈或DMA，所提整体峰值RAM增量目标仍需实机测量。新0.6.1实机测试为**NOT RUN**；需检查大小声回答、停顿、取消、菜单/熄屏继续运行、暖机重进和长聊。界面改善不能证明旧Opus分配故障已修复。
